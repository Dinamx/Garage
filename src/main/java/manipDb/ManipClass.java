...
//                        parametreIndex  = parametreIndex;
//                    }
//                    Dina solona if de tsoaana le eo amle id etsy ambony etsy raha tia anao sequence
//                    else if (!attributes[i].equals("id")) {
...
//                    }
    ...
    public static void fillStatementReturning(PreparedStatement statement, Class objClass, Object[] values) {
        HashMap<String, Class> attribTypes = getAttributes(objClass);
        Set<String> keys = attribTypes.keySet();
        String[] attributes = keys.toArray(new String[keys.size()]);
        try {
...
//                    }
    ...
    public static java.util.Date toDate(Date daty) {
    ...
    public static HashMap<String, Object> getConditionValues(Object filtre) {
        ...
        try {
            for (int i = 0; i < methods.length; i++) {
                rawValues[i] = methods[i].invoke(filtre);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
